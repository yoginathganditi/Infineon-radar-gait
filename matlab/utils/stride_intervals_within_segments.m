function stride_times = stride_intervals_within_segments(peaks, segments, frame_dt)
    % STRIDE_INTERVALS_WITHIN_SEGMENTS Compute stride intervals within walking segments
    % Inputs:
    %   peaks: array of peak frame indices
    %   segments: cell array of [start, end] frame pairs
    %   frame_dt: frame time interval (s)
    % Output:
    %   stride_times: array of stride intervals (s)
    ...
    for i = 1:length(seg_peaks)
        ...
        if length(p) >= 3
            ...
            intervals = (p(3:end) - p(1:end-2)) * frame_dt;
            intervals = intervals(:)';  % Make row vector
            dts = [dts, intervals];
        end
    end
    
    stride_times = dts(:);  % Return as column vector
end
